...
bool TemplatedVocabulary<TDescriptor, F>::loadFromTextFile(const std::string& filename)
{
    ...
    while (!f.eof())
    {
        ...
        int pid;
        ssnode >> pid;
        fwrite(&pid, 1, sizeof(int), fpnodes);
        m_nodes[nid].parent = pid;
        m_nodes[pid].children.push_back(nid);

        int nIsLeaf;
        ssnode >> nIsLeaf;
        fwrite(&nIsLeaf, 1, sizeof(int), fpnodes);
        ...
        F::fromString(m_nodes[nid].descriptor, ssd.str());
        //class FORB: protected FClass { typedef cv::Mat TDescriptor; // CV_8U, Descriptor type }
        fwrite(m_nodes[nid].descriptor.ptr<unsigned char>(), 32, sizeof(char), fpnodes);

        ssnode >> m_nodes[nid].weight;
        fwrite(&m_nodes[nid].weight, 1, sizeof(double), fpnodes);
    ...
    for (int i = 0; i < 1082074; i++)
    {
        ...
        int pid;
        //ssnode >> pid;
        fread(&pid, 1, sizeof(int), fpnodes);
        m_nodes[nid].parent = pid;
        m_nodes[pid].children.push_back(nid);

        int nIsLeaf;
        //ssnode >> nIsLeaf;
        fread(&nIsLeaf, 1, sizeof(int), fpnodes);
        ...
        //F::fromString(m_nodes[nid].descriptor, ssd.str());
        //class FORB: protected FClass { typedef cv::Mat TDescriptor; // CV_8U, Descriptor type }
        m_nodes[nid].descriptor.create(1, 32, CV_8U);// FORB::L = 32
        fread(m_nodes[nid].descriptor.ptr<unsigned char>(), 32, sizeof(char), fpnodes);

        //ssnode >> m_nodes[nid].weight;
        fread(&m_nodes[nid].weight, 1, sizeof(double), fpnodes);
        ...
        {
            m_nodes[nid].children.reserve(m_k); // reserve 10 positions for children if not a leaf node
        }
    }
#endif
